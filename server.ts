...
async function sendMail(to: string, subject: string, html: string, replyTo?: string) {
  ...
  try {
    ...
    console.log(`[Mail Server] Email successfully sent to ${to} for "${subject}"`);
    return true;
  } catch (err) {
...
async function startServer() {
  ...
  // =====================
  // API: Contact Form Submission (Mail Server)
  // =====================
  app.post('/api/contact', async (req, res) => {
    ...
    // Send email to store support inbox
